...
internal class HtmlElement : IElement
{
    ...
    public string? Html { get => $@"<html><title>{Title}</title><meta name=""viewport"" content=""width=device-width, initial-scale=1.0"">{Children.Html()}</html>"; }
    public string? Title { get; init; }
}
...
internal sealed class StringBuildRequestHandler : IRequestHandler<StringBuildRequest, StringBuildResponse>
{
    ...
    private static string? Build(string? source)
    {
        ...
        {
            return @$"<cite id=""cite-{match.Groups["CITE_INDEX"].Value}""><a href=""#cited-{match.Groups["CITE_INDEX"].Value}"">({match.Groups["CITE_INDEX"].Value})</a>. {match.Groups["CITE_CONTENT"].Value}</cite>";
        }, RegexOptions.Multiline);
