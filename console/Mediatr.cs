...
internal sealed class StringBuildRequestHandler : IRequestHandler<StringBuildRequest, StringBuildResponse>
{
    ...
    const string NOTE = @"(?'NOTE'(?'NOTE_START'<blockquote><p>)(?'NOTE_CONTENT'\[!NOTE\]))";
    const string TIP = @"(?'TIP'(?'TIP_START'<blockquote><p>)(?'TIP_CONTENT'\[!TIP\]))";
    const string IMPORTANT = @"(?'IMPORTANT'(?'IMPORTANT_START'<blockquote><p>)(?'IMPORTANT_CONTENT'\[!IMPORTANT\]))";
    const string WARNING = @"(?'WARNING'(?'WARNING_START'<blockquote><p>)(?'WARNING_CONTENT'\[!WARNING\]))";
    const string CAUTION = @"(?'CAUTION'(?'CAUTION_START'<blockquote><p>)(?'CAUTION_CONTENT'\[!CAUTION\]))";
    ...
    private static string? Build(string? source)
    {
        ...
        target = Regex.Replace(target, @$"({NOTE})", (match) =>
        {
            return $"{match.Groups["NOTE_START"]}{SvgCreate(
                "#1f6feb", 
        ...
        target = Regex.Replace(target, @$"({TIP})", (match) =>
        {
            return $"{match.Groups["TIP_START"]}{SvgCreate(
                "#3fb950", 
                "M8 1.5c-2.363 0-4 1.69-4 3.75 0 .984.424 1.625.984 2.304l.214.253c.223.264.47.556.673.848.284.411.537.896.621 1.49a.75.75 0 0 1-1.484.211c-.04-.282-.163-.547-.37-.847a8.456 8.456 0 0 0-.542-.68c-.084-.1-.173-.205-.268-.32C3.201 7.75 2.5 6.766 2.5 5.25 2.5 2.31 4.863 0 8 0s5.5 2.31 5.5 5.25c0 1.516-.701 2.5-1.328 3.259-.095.115-.184.22-.268.319-.207.245-.383.453-.541.681-.208.3-.33.565-.37.847a.751.75 0 0 1-1.485-.212c.084-.593.337-1.078.621-1.489.203-.292.45-.584.673-.848.075-.088.147-.173.213-.253.561-.679.985-1.32.985-2.304 0-2.06-1.637-3.75-4-3.75ZM5.75 12h4.5a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1 0-1.5ZM6 15.25a.75.75 0 0 1 .75-.75h2.5a.75.75 0 0 1 0 1.5h-2.5a.75.75 0 0 1-.75-.75Z"
            )}";
        }, RegexOptions.Multiline);

        target = Regex.Replace(target, @$"({IMPORTANT})", (match) =>
        {
            return $"{match.Groups["IMPORTANT_START"]}{SvgCreate(
                "#ab7df8", 
        ...
        target = Regex.Replace(target, @$"({WARNING})", (match) =>
        {
            return $"{match.Groups["WARNING_START"]}{SvgCreate(
                "#d29922", 
                "M6.457 1.047c.659-1.234 2.427-1.234 3.086 0l6.082 11.378A1.75 1.75 0 0 1 14.082 15H1.918a1.75 1.75 0 0 1-1.543-2.575Zm1.763.707a.25.25 0 0 0-.44 0L1.698 13.132a.25.25 0 0 0 .22.368h12.164a.25.25 0 0 0 .22-.368Zm.53 3.996v2.5a.75.75 0 0 1-1.5 0v-2.5a.75.75 0 0 1 1.5 0ZM9 11a1 1 0 1 1-2 0 1 1 0 0 1 2 0Z"
            )}";
        }, RegexOptions.Multiline);

        target = Regex.Replace(target, @$"({CAUTION})", (match) =>
        {
            return $"{match.Groups["CAUTION_START"]}{SvgCreate(
                "#f85149", 
