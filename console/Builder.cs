...
internal class Html : IElement
{
    ...
    public string? Built { get => $@"<html><head><title>{Title}</title><meta name=""viewport"" content=""width=device-width, initial-scale=1.0""><style>html {{ display:flex; justify-content:center; }} html > body {{ display:flex; flex-direction:column; max-width: 1024px; flex:1 1 auto; }}</style></head>{Children.Build()}</html>"; }
    public string? Title { get; init; }
}
