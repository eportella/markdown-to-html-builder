...
internal sealed partial class ABuildRequestHandler(Task<ProjectBuildResponse> projectTask) : IRequestHandler<ABuildRequest, string?>
{
        ...
        var target = await Regex().ReplaceAsync(
            ...
            async match =>
            {
                ...
                if (string.IsNullOrWhiteSpace(href.Host))
                    return $@"<a href=""{(await projectTask).BaseUrl!.AbsoluteUri.TrimEnd('/')}/{href.LocalPath.TrimStart('/')}"">{match.Groups["A_CONTENT"].Value}</a>";
