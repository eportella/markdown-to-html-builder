...
internal sealed class SvgBuildRequestHandler() : IRequestHandler<SvgBuildRequest, string?>
{
    ...
    static Regex RegexNote { get; }
    const string PATTERN_TIP = @"(?'SVG_TIP'\[!TIP\])";
    static Regex RegexTip { get; }
    const string PATTERN_IMPORTANT = @"(?'SVG_IMPORTANT'\[!IMPORTANT\])";
    static Regex RegexImportant { get; }
    const string PATTERN_WARNING = @"(?'SVG_WARNING'\[!WARNING\])";
    static Regex RegexWarning { get; }
    const string PATTERN_CAUTION = @"(?'SVG_CAUTION'\[!CAUTION\])";
    static Regex RegexCaution { get; }

    static SvgBuildRequestHandler()
    {
        RegexNote = new Regex(PATTERN_NOTE, RegexOptions.Multiline);
        RegexTip = new Regex(PATTERN_TIP, RegexOptions.Multiline);
        RegexImportant = new Regex(PATTERN_IMPORTANT, RegexOptions.Multiline);
        RegexWarning = new Regex(PATTERN_WARNING, RegexOptions.Multiline);
        RegexCaution = new Regex(PATTERN_CAUTION, RegexOptions.Multiline);
    }
    public async Task<string?> Handle(SvgBuildRequest request, CancellationToken cancellationToken)
    {
        ...
        target = await RegexNote.ReplaceAsync(
            target,
            match => $@"<span class=""icon"">{SvgBuild(
                    "var(--color-note-a50)",
        ...
        target = await RegexTip.ReplaceAsync(
            target,
            match => $@"<span class=""icon"">{SvgBuild(
                    "var(--color-tip-a50)",
        ...
        target = await RegexImportant.ReplaceAsync(
            target,
            match => $@"<span class=""icon"">{SvgBuild(
        ...
        target = await RegexWarning.ReplaceAsync(
            target,
            match => $@"<span class=""icon"">{SvgBuild(
        ...
        target = await RegexCaution.ReplaceAsync(
            target,
            match => $@"<span class=""icon"">{SvgBuild(
